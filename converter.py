...
def convert(config_path, weights_path, output_path, retrain=False, num_classes=None):
	...
	with graph.as_default():
		...
		momentum = float(cfg_parser['net_0']['momentum']
			) if 'net_0' in cfg_parser.sections() else 0.9

		initializer = tf.contrib.layers.xavier_initializer()

		input_layer = tf.placeholder(dtype='float32', shape=[None, None, None, 3], name='Input')
		# input_ = tf.image.resize_images(input_layer, [608, 608], method=ResizeMethod.BICUBIC,
		...
		for section in cfg_parser.sections():
				...
				with tf.name_scope(section):
					...
					if batch_normalization:
						with tf.name_scope('batch_norm'):
							...
							# 	true_fn=lambda: train(conv_layer, beta, gamma), 
							# 	false_fn=lambda: valtest(conv_layer, moving_mean, moving_variance, 
							# 		beta, gamma))
							...
							conv = tf.nn.batch_normalization(x=conv_layer, 
										mean=mean, variance=variance, 
										offset=beta, scale=gamma, 
										variance_epsilon=1e-3)
			...
			# Parsing Shortcut/Res Layer
				...
				prev_layer = all_layers[-1]
				# if retrain and layer_count==out_layers[0]-1:
