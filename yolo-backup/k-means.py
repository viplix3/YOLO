...
def avg_iou(boxes, clusters):
	""" Calculates average IoU between the GT boxes and clusters 
		Input:
			boxes: numpy array having width and height of all the GT boxes
		Output:
	...
	with open(file_path, 'r') as f:
		...
		for line in f:
			...
			for i in range(1, length):
				width = int(infos[i].split(',')[2]) - int(infos[i].split(',')[0]) # x2 - x1
				height = int(infos[i].split(',')[3]) - int(infos[i].split(',')[1]) # y2 - y1
...
if __name__ == '__main__':
	...
	clusters = np.arange(9, 10, dtype=int)
	avg_accuracy = []
	for i in clusters:
