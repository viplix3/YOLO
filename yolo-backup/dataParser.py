...
class Parser:
	...
	def read_annotations(self, file_path):
		...
		with open(file_path) as file:
			for lines in file.read().splitlines():
				...
				file_name.append(os.path.join('./dataset', line[0]))
				line = line[1::]
				_BB = []
				_class_id = []
	...
	def make_tfrecord(self):
		...
		file_path = file_path[:2000]
		bounding_boxes = bounding_boxes[:2000]
		...
		config.train_num = train_split
		config.val_num = val_split
