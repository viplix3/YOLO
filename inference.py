...
parser.add_argument('--iou_thresh', help="IoU for considering a BB TP", default="0.45")
...
parser.add_argument('--iou_threshold', help='Threshld IoU for applying NMS', default=0.3)
...
def run_inference(checkpoint_path, img_path, dataset, args):
	...
	for x in range(len(images_batch)):
		...
		out_boxes, out_scores, out_classes = sess.run(predict(output_nodes, anchors, num_classes, 
			input_shape, image_shape, args), feed_dict={input_node: img, is_training: False})
